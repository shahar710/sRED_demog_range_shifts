...
##Bioshift
```{r}
bioshifts_harmo_intersect=read_rds("C:\\Users\\User\\Desktop\\research\\working groups\\iDiv_2024_25_sRED\\R\\sRED_demog_range_shifts\\stage_2_merge_data/bioshifts_harmo_intersect.rds")
```

species by ecosystem
```{r}
sp_by_ecosystem=bioshifts_harmo_intersect %>% 
  group_by(Ecosystem) %>% 
...
```{r}
sp_by_phylum_class=bioshifts_harmo_intersect %>% 
...
```{r}
sp_by_ecosystem_class=bioshifts_harmo_intersect %>% 
...
```{r}
bioshifts_harmo_intersect$Position=factor(bioshifts_harmo_intersect$Position,levels = c("Trailing edge","Centroid","Leading edge"))
ggplot(bioshifts_harmo_intersect)+
